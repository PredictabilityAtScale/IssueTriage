...
const signState = async (secret, value) => {
...
const packState = async (secret) => {
    const nonce = crypto.randomUUID();
    const signature = await signState(secret, nonce);
    return `${nonce}.${signature}`;
};
const verifyState = async (secret, state) => {
...
router.post("/oauth/exchange", async (request, env) => {
    const clientId = env.ISSUETRIAGE_GITHUB_CLIENT_ID;
    const clientSecret = env.ISSUETRIAGE_GITHUB_CLIENT_SECRET;
    if (!clientId || !clientSecret) {
        return new Response("GitHub OAuth not configured", { status: 500 });
...
router.post("/llm", async (request, env) => {
