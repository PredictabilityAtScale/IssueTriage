...
export class AssessmentStorage {
	...
	private async openDatabase(): Promise<void> {
		...
		if (existing) {
			try {
				...
				console.error('[IssueTriage] Assessment database is corrupt, backing up and recreating.', err);
				if (this.db) {
	...
	private async backupCorruptDatabase(): Promise<void> {
		try {
			...
			console.warn(`[IssueTriage] Corrupt assessment DB backed up to ${backupPath}`);
		} catch (backupErr) {
			console.error('[IssueTriage] Failed to back up corrupt assessment DB.', backupErr);
